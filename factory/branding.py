...
def create_intro_slide(title: str, logo_path: str, output_path: str, width: int, height: int) -> Optional[str]:
    ...
    try:
        ffmpeg_cmd = [
            ...
            # Scale logo and overlay with alpha fade-in effect
            f"[1:v]scale={logo_size}:{logo_size}[logo_scaled];"
            f"[with_title][logo_scaled]overlay=x={logo_x}:y={logo_y}:enable='between(t,0,3)':eval=frame",
...
def create_outro_slide(logo_path: str, output_path: str, width: int, height: int) -> Optional[str]:
    ...
    try:
        ffmpeg_cmd = [
            ...
            # Scale logo and overlay with alpha fade-in effect
            f"[1:v]scale={logo_size}:{logo_size}[logo_scaled];"
            f"[with_text][logo_scaled]overlay=x={logo_x}:y={logo_y}:enable='between(t,0,3)':eval=frame",
